...
This repository is the source code of the Dpowcoin Core project website built with Jekyll.

## Directory structure

  - `_posts/<lang>/posts` for blog articles.
  - `_posts/<lang>/pages` for static pages.
  - `_posts/<lang>/releases` for Dpowcoin Core release notes
...
The following `Front Matter` is required for the multilingual setup in all files. The required fields are as follows:
  ...
  - `type:`      the content type (`pages`, `posts`, `releases`, etc.)
...
Then clone this repository and change directory into it:
    ...
    git clone https://github.com/dpowcore-project/dpowcore.org.git
    cd dpowcore.org
