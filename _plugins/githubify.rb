...
require 'yaml'
  ...
  class GitHubifyBlock < Liquid::Block
    ...
    def render(context)
      ...
      ## other monotree repos Require at least a
      ...
      ## Other monotree repos; for repo "dpowcore-project/foo", PR format is "foo#123".
      ## Must start at word boundary
      output.gsub!(/\b([a-z]+)#([0-9]+)/){ |s|
        '<a href="https://github.com/dpowcore-project/' + $1 + '/pull/' + $2 + '">' + s + '</a>'
      }
