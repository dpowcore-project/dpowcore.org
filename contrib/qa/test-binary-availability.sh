...
# Local builds and Travis CI builds of the dpowcore.org website do
# not include the /bin directory holding dpowcoin Core binaries, so this
...
do
  ...
  if [ "${url##http*}" ]
  then
    curl -sI "https://dpowcore.org${url}"
  else
    curl -sI "$url"
