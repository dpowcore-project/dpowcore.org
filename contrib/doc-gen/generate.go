// This is a golang script, needed for generating the RPC dpowcoin documentation
//
// What is necessary to run this:
// (1) install golang
// (2) install dpowcoin core, set it up to use regtest
// (3) run dpowcoind
// (4) run this script with `go run generate.go` while being in contrib/doc-gen, and with dpowcoin-cli in PATH
...
const BITCOIN_COMMAND = "dpowcoin-cli"
...
func run(args ...string) string {
	...
	if err != nil {
		log.Fatalf("Cannot run dpowcoin-cli: %s, is dpowcoind (regtest) running?", err.Error())
	}
